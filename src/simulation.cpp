...
Simulation::Simulation(int num_particles, double maxX, double maxY) {
  for(size_t i = 0; i < (unsigned) num_particles; ++i) {
    particles.push_back(Particle(radius, (std::rand() % 10 / 10.0) * (maxX - 2 * radius) + radius, (std::rand() % 10 / 10.0) * (maxY - 2 * radius) + radius, ((std::rand() % 10 / 10.0) * maxX)/velPosScale, ((std::rand() % 10 / 10.0) * maxY)/velPosScale));
  }
...
Particle& Simulation::handleCollision(Particle& particle) {
  ...
  for(size_t i = 0; i < particles.size(); ++i) {
    ...
    if(!checkEqual(particle, other) && (particleDistance(particle, other)) <= radius * 2 && moveTowards(particle, other)){
      glm::vec2 newParticleVel = particleCollision(particle, other);
      particles[i].setVelocity(particleCollision(other, particle).x, particleCollision(other, particle).y);
      particle.setVelocity(newParticleVel.x, newParticleVel.y);
    }
  }
...
glm::vec2 & Simulation::particleCollision(Particle &particle1, Particle &particle2) {
...
glm::vec2 posCheck(Particle& particle, double maxX, double maxY) {
  ...
  if(pos.x - radius < 0) {
    ...
    pos.x = (float) (maxX - radius);
  }

  if(pos.y - radius < 0) {
    pos.y = (float) radius;
  }else if(pos.y + radius > maxY) {
    pos.y = (float) (maxY - radius);
  }
